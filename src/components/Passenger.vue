<template>
  ...
  <div class="mr-2 my-2 border-2  rounded-md" :class="isPassengerPanelValid ? 'border-lime-500' : 'border-orange-200'">
    ...
    <form :id="`passengerForm_${index}`"
      ...
      <header
        ...
        <svg v-if="index === 1" class="w-6 h-6  flex-none" xmlns="http://www.w3.org/2000/svg" fill="none"
          viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round"
            d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" />
        </svg>

        <div clas="grow font-thin" v-if="index === 1" :class="index === 1 ? 'pl-2' : ''">
          <span class="font-bold">{{ state.name }}</span>
          (Contact Passenger)
        </div>

        <div clas="grow" v-if="index > 1" :class="index === 1 ? 'pl-2' : ''">
          Passenger #{{ index }}
        </div>

        <svg v-if="isPassengerPanelValid" class="flex-none w-6 h-6 fill-lime-500  " xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24" fill="currentColor">
          <path fill-rule="evenodd"
            d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12zm13.36-1.814a.75.75 0 10-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 00-1.06 1.06l2.25 2.25a.75.75 0 001.14-.094l3.75-5.25z"
            clip-rule="evenodd" />
        </svg>
        <svg v-if="!isPassengerPanelValid" class="flex-none  w-6 h-6  fill-red-700  " xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24" fill="currentColor">
          <path fill-rule="evenodd"
      ...
      <div :id="`passengerInputs_${index}`">
        ...
        <!-- Passenger's Name  -->
        <div class="px-6 pt-2">
          <label v-if="index === 1" :for="`contactName_${index}`"
            class="mt-2 block text-sm font-medium leading-6 text-gray-900">First &amp; Last Name</label>
          <label v-if="index > 1" :for="`contactName_${index}`"
            class="mt-2 block text-sm font-medium leading-6 text-gray-900">Name</label>
          <div class="relative mt-1 rounded-md shadow-sm">
            ...
            <input v-model="state.name" type="text" :name="`contactName_${index}`" :id="`contactName_${index}`" class="block w-full rounded-md border-0 py-2.5 pl-11 
								ring-1 ring-inset  placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 
								sm:text-sm sm:leading-6"
              :class="(v$.name.$invalid && v$.name.$dirty) ? 'text-red-700  ring-red-700' : 'text-gray-900  ring-gray-300'"
              :placeholder="index === 1 ? 'Contact First, Last Name' : 'Name'" @focusout="v$.name.$touch" />
          ...
          <p v-if="index === 1 && (v$.name.$invalid && v$.name.$dirty)" :id="`contact-warning_${index}`"
        ...
        <div class="flex justify-around pt-6">
          ...
          <fieldset class="mt-2 mb-2">
            ...
            <div class="flex flex-center mb-4">
              <input :id="`M_Check_${index}`" :name="`Sex_Check_${index}`" type="radio" v-model="state.sex" value="male"
                @focusout="sexTouched = true" :checked="state.sex === 'male'"
                class="h-6 w-6  border-2 border-gray-400 text-indigo-600 focus:ring-gray-300" />
              <label :for="`M_Check_${index}`" class="ml-2 block font-medium leading-6 text-lg"
            ...
            <div class="flex flex-center">
              <input :id="`FCheck_${index}`" :name="`Sex_Check_${index}`" type="radio" v-model="state.sex"
                value="female" @focusout="sexTouched = true" :checked="state.sex === 'female'"
                class="h-6 w-6 border-2 border-gray-400 text-indigo-600 focus:ring-gray-300" />
              <label :for="`FCheck_${index}`" class="ml-2 block font-medium leading-6 text-gray-900 text-lg"
                :class="(sexTouched === false || v$.sex.$invalid === false) ? 'text-gray-900' : 'text-red-700'">
                Female
              </label>
            </div>
          ...
          <div class="custom-number-input h-10 w-32">
            ...
            <div class="flex flex-row h-10 w-full rounded-lg relative bg-transparent mt-1">
              <button @click="increment(-1, $event)" class="plusButtn 
              ...
              <input :id="`Age_${index}`" type="text" inputmode="numeric" class="z-[0]  ring-gray-400 ring-inset border-0 focus:outline-none text-center w-full 
                ...
                style="text-shadow: 1px 1px 2px black/10;" name="custom-input-number" autocomplete="off"
                v-model="ageInt" placeholder="Age" @change="onChanged" />

              <button @click="increment(1, $event)" class="minusButton 
                  bg-gradient-to-b from-gray-100 to-gray-300
                  border-l-0 border-[1px] border-gray-400
                  bg-gray-200 text-gray-800 
                  hover:text-black hover:from-gray-200 hover:to-gray-400 
                  h-full w-20 rounded-r-md cursor-pointer" :class="ageInt >= maxVal ? 'opacity-40' : 'opacity-100'">
                <span class="m-auto text-2xl font-medium">+</span>
              </button>
            </div>
            <p v-if="v$.age.$invalid && ageTouched === true" :id="`age-error-warning_${index}`"
              class="mt-2 px-0 absolute italic text-sm text-red-400">
              Your Age is required.
            </p>
            <p v-if="(ageInt <= minVal) && v$.age.$invalid === false" :id="`age-min-warning_${index}`"
              class="mt-2 px-0 absolute italic text-sm text-gray-700">
              5 years is the minimum.
            </p>
            <p v-if="ageInt >= maxVal" :id="`age-max-warning_${index}`"
              class="mt-2 px-0 absolute italic text-sm text-gray-700">
...
watch(phoneNumber, (newValue, oldValue) => {
  if (oldValue !== undefined) {       // Bug FIX!!! This is catching a mismatch on form load, that is deleting all the cached data. Aug 2024 update.
    if (newValue !== oldValue) {
