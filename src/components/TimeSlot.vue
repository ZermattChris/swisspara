<template>
  ...
  <div class="px-2 py-1.5 rounded-lg "
    ...
    <div class="time_slot_header py-1 mb-2 text-xl  rounded-md text-center font-black   border-black/30 border-2"
      ...
      <span v-if="isFlightSlide" class="absolute  right-2 rotate-45" :class="isSlideSelected ? 'top-2' : 'top-5'">
        <svg class="fill-white drop-shadow-md" xmlns="http://www.w3.org/2000/svg" height="48" width="48"
      ...
      <span class="font-normal inline-block w-full text-lg ">
        {{ formatHeaderDate }}
    ...
    <ul role="list" class="space-y-1 py-1 ">
      ...
      <li v-for="(pilots, timeHint, index) in dayObject" :key="pilots.id"
        ...
        <span v-if="isSlideSelected && selectedSlot == index && (pilots[0] > 0)"
          ...
          <button :id="`plusBtn_${slideIndex}_${index}`" class="rounded-full bg-amber-500 shadow-md shadow-black/50"
